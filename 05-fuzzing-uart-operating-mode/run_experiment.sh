...
TARGET_LIST="interrupt-mode/Curiosity_PIC32MX_FreeRTOS interrupt-mode/DUINOMITE_MEGA interrupt-mode/StarterKit_PIC32MZ interrupt-mode/StarterKit_PIC32MZ_FreeRTOS"
TARGET_LIST="$TARGET_LIST polling-mode/Curiosity_PIC32MX_FreeRTOS polling-mode/DUINOMITE_MEGA polling-mode/StarterKit_PIC32MZ polling-mode/StarterKit_PIC32MZ_FreeRTOS"
...
$DIR/run_genstats.sh 1 $EXPERIMENT_REPETITION_COUNT $NUM_PARALLEL_INSTANCES $FUZZING_RUNTIME $TARGET_LIST

if [ $SKIP_NON_MODELING -ne 1 ]; then
    # Run all targets without modeling
    $DIR/run_genstats.sh 0 $EXPERIMENT_REPETITION_COUNT $NUM_PARALLEL_INSTANCES $FUZZING_RUNTIME $TARGET_LIST
fi
